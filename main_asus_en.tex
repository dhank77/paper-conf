...
To contextualize SyncPilot's performance, we compare against a serial FSRCNN execution baseline on a pinned Performance core (10288 ms). Furthermore, to validate our claim against the default OS scheduler, we also compare against CFS Baselines where 10 and 20 threads are spawned without explicit CPU affinity, leaving the Linux Completely Fair Scheduler (aided by Energy Aware Scheduling) to manage core allocation freely; we additionally stress-test this comparison under synthetic system noise in Section~\ref{sec:discussion}. All results in this section (Tables~\ref{tab:throughput}--\ref{tab:noise}) come from a single benchmark session using the same 8-run protocol, with run 1 of every scenario treated as a cold-start/profiling run and excluded from the reported average, minimum, maximum, and standard deviation, which are computed over the remaining 7 steady-state runs; run 1 typically differs from steady state by cold page cache (weight files, input video read twice) and thread/engine initialization, not by anything IC-RCE-specific.
...
\hline
\textbf{Config.} & \textbf{Time (ms)} & \textbf{StdDev (ms)} & \textbf{FPS} & \textbf{Speedup} \\
\hline
Serial (Performance Core) & 10288 & 139.0 & 14.58 & 1.00$\times$ \\
SyncPilot-4W & 3026 & 15.0 & 49.57 & 3.40$\times$ \\
SyncPilot-8W & 1583 & 9.0 & 94.76 & 6.50$\times$ \\
SyncPilot-10W & 1340 & 9.2 & 111.94 & 7.68$\times$ \\
SyncPilot-20W & 1123 & 16.9 & 133.57 & 9.16$\times$ \\
\hline
\end{tabular}
...
A key contribution of SyncPilot is that IC-RCE calibration happens only once, during the processing of the first task, rather than being re-measured on every frame. Empirically, the observed cold-start delta on this benchmark session is modest rather than dramatic: SyncPilot-4W's first (cold-start) run completed in 3036 ms versus a 3026 ms steady-state average (+0.3\%), and SyncPilot-20W's first run completed in 1128 ms versus a 1123 ms steady-state average (+0.4\%); for SyncPilot-8W, the first run was in fact marginally \emph{faster} than the steady-state average (1573 ms vs. 1583 ms). We do not observe a large, multi-second one-time penalty; the most likely explanation is that the weight and input files were already resident in the OS page cache from compilation and the preceding scenarios within the same benchmark session, so the ``cold cache'' condition a first run is meant to stress-test was only partially realized here. A genuinely cold-cache measurement (e.g., immediately after a reboot, before any scenario has touched the files) is left as follow-up work. What the architecture guarantees independent of this empirical variability is structural rather than measured: once the Cost Table is populated after Task ID 0, the scheduling logic performs a constant-time table lookup instead of invoking \texttt{clock\_gettime} again, so per-task dispatch overhead does not grow with the number of frames processed — this is a property of the code path itself, not an artifact of any particular timing run.
...
Note that this single-frame calibration total (94.32~ms) is noticeably higher than the amortized steady-state per-frame cost implied by the serial baseline (10288~ms / 150~frames $\approx$ 68.6~ms/frame, Table~\ref{tab:throughput}) — a gap of roughly 38\%. This per-layer Cost Table was captured separately from the timing runs in Table~\ref{tab:throughput} (Section~\ref{sec:calibrationoverhead}) and was not part of the corrected-methodology rerun, so it should be read as illustrating the \emph{relative} cost distribution across layers (Layer 8 dominance) rather than as a millisecond-exact figure directly comparable run-for-run with Table~\ref{tab:throughput}; unlike the whole-run cold-start deltas reported in Section~\ref{sec:calibrationoverhead}, which turned out to be small, we cannot rule out that this specific single-frame measurement still carries some cold-cache or \texttt{clock\_gettime}-instrumentation cost. We do not have independent per-frame instrumentation of the steady-state serial run to confirm the 68.6~ms/frame figure directly (only its 150-frame aggregate); resolving this gap, and re-capturing the Cost Table under the corrected methodology, is left as verification work.
...
Output correctness was verified across all SyncPilot and CFS-baseline configurations, including under the noise conditions of Section~\ref{sec:noise}, and compared with the reference paper's findings. Every one of the 150 output frames in every one of these scenarios (SyncPilot-4W/8W/10W/20W, the 5B5L split, CFS-10W/20W, and both SyncPilot-20W and CFS-20W under 4- and 10-thread noise) was bit-exact against the ground truth, confirmed by pixel-wise comparison (0 differing bytes) and infinite PSNR. We report this as an expected consequence of FSRCNN's deterministic arithmetic combined with the Reorder Buffer's sequencing guarantee, rather than as a surprising finding on its own; its value is as a direct contrast to \cite{annisa2025fsrcnn}, which reported that the Efficient CPU parallel scenario produced visible artifacts and pixel corruption due to race conditions during the deconvolution stage. SyncPilot's synchronized worker pool with reorder buffer rules out that class of race condition by construction, ensuring bit-exact outputs regardless of the worker configuration. Section~\ref{sec:racecond} reproduces the corrupted-output failure mode itself on our own platform for direct comparison.
...
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Config.} & \textbf{Time (ms)} & \textbf{StdDev} & \textbf{FPS} & \textbf{Speedup} \\
\hline
Serial (Performance Core) & 10288 & 139.0 & 14.58 & 1.00$\times$ \\
SyncPilot-10W & 1340 & 9.2 & 111.94 & 7.68$\times$ \\
CFS-10W (No Affinity) & 1332 & 6.8 & 112.61 & 7.72$\times$ \\
SyncPilot-20W & 1123 & 16.9 & 133.57 & 9.16$\times$ \\
CFS-20W (No Affinity) & 1039 & 16.4 & 144.37 & 9.90$\times$ \\
\hline
\end{tabular}
...
\hline
\textbf{Noise} & \textbf{Config.} & \textbf{Avg (ms)} & \textbf{StdDev} & \textbf{Worst-Case Run (ms)} & \textbf{Speedup*} \\
\hline
\multirow{2}{*}{Idle} & SyncPilot-20W & 1123 & 16.9 & 1154 & 9.16$\times$ \\
 & CFS-20W & 1039 & 16.4 & 1065 & 9.90$\times$ \\
\hline
\multirow{2}{*}{4 Threads} & SyncPilot-20W & 1219 & 18.1 & 1248 & 8.44$\times$ \\
 & CFS-20W & 1275 & 19.1 & 1314 & 8.07$\times$ \\
\hline
\multirow{2}{*}{10 Threads} & SyncPilot-20W & 1403 & 15.8 & 1426 & 7.33$\times$ \\
 & CFS-20W & 1735 & 45.1 & 1821 & 5.93$\times$ \\
\hline
\multicolumn{6}{l}{\footnotesize *All speedups relative to the single 10288~ms idle serial baseline (Table~\ref{tab:throughput});}\\
\multicolumn{6}{l}{\footnotesize serial execution was not separately re-measured under noise.}\\
\end{tabular}
\end{adjustbox}
\label{tab:noise}
\end{center}
...
Table \ref{tab:noise} shows a reversal of the idle-system result. Under moderate noise (4 threads), SyncPilot-20W already overtakes CFS-20W in both average latency (1219 ms vs. 1275 ms) and worst-case run latency (1248 ms vs. 1314 ms). Under heavy noise (10 threads), the gap widens substantially in absolute terms: SyncPilot-20W's worst-case run latency grows from 1154 ms (idle) to 1426 ms, while CFS-20W's grows from 1065 ms (idle) to 1821 ms — SyncPilot remains faster in the worst case despite starting from a slower idle baseline. Expressed relative to each configuration's own idle baseline, this is a 23.6\% increase for SyncPilot-20W versus 71.0\% for CFS-20W; we report the absolute figures as the primary result and the relative growth only as a secondary, normalized view, since the relative framing alone could be read as inflating the comparison. The standard deviation column reinforces the same pattern: SyncPilot-20W's run-to-run StdDev stays in a narrow 15.8--18.1 ms band across all three noise conditions, while CFS-20W's StdDev nearly triples under heavy noise (16.4~$\to$~45.1 ms), indicating that CFS's added variability under load is not just a shift in mean latency but a genuine loss of run-to-run predictability.
...
\label{sec:racecond}
A significant finding from \cite{annisa2025fsrcnn} was that parallel FSRCNN execution on Efficient CPUs produced visible artifacts and pixel corruption due to race conditions during the deconvolution stage. Their serial execution produced artifact-free frames, while the parallel Efficient CPU scenario showed degraded image quality. Rather than relying solely on this citation, we reproduced the same failure mode directly on our own platform: an unsynchronized, naively-parallelized OpenMP baseline (\texttt{fsrcnn\_naive\_openmp}, 20 threads, no reorder buffer or stage-level synchronization) run against the same input and compared against our deterministic \texttt{fsrcnn\_serial} ground truth. This NAIVE scenario differed from the ground truth in 10{,}709{,}111 of 22{,}809{,}600 output bytes (46.9\%), with an average PSNR of 22.37~dB (20.61~dB on the luminance channel) — a clear, quantified reproduction of the race-condition corruption reported in \cite{annisa2025fsrcnn}, obtained on our own hardware and input rather than taken on faith from the reference paper. In contrast, every SyncPilot and CFS-baseline configuration in Table~\ref{tab:throughput}--\ref{tab:noise} produced bit-exact outputs against the same ground truth across all 150 frames (0 differing bytes; infinite PSNR). This improvement stems from SyncPilot's synchronized architecture: the pull-based worker pool with shared stage queues and reorder buffer ensures that each stage's output is correctly sequenced before being consumed by the next stage, structurally ruling out the race conditions that both the reference paper's OpenMP-based parallelization and our own NAIVE reproduction exhibit.
