...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/architecture_pipeline.pdf}
...
\footnotesize
\emph{Speedup vs. Serial}: CPU rows against their own variant's 1-thread time; GPU V2 has none of its own, so its denominator is CPU V1's serial time on that platform (Section~\ref{sec:gb10-speedup} also reports it against the faster bit-exact V0 serial time). \emph{Speedup vs. Best CPU}: GPU V2 only, against CPU V1's fastest correctness-verified configuration. \emph{Dev./SSIM} are against each platform's own deterministic reference; ``bit-exact'' is logically equivalent to \texttt{cmp -l}, not an independent quality measurement, and figures are all-component averages (Section~\ref{sec:methodology}). \emph{GPU L8 Kernel}: mean $\pm$ SD over 6 runs of the profiling sweep (Section~\ref{sec:breakdown}); kernel times for the three remaining grid configurations are given in Section~\ref{sec:warp-generalize}.
\end{minipage}
\end{table*}
