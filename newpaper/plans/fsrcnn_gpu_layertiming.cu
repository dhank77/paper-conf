// ============================================================================
// PER-LAYER TIMING VARIANT -- derived mechanically from fsrcnn_gpu_instrumented.cu,
// which is itself derived from fsrcnn_gpu_main.cu. Numerics are untouched.
//
// Adds a wall-clock timer around each of CPU Layers 1-7 individually, on top of
// the per-kernel GPU events already present. Purpose: Table III reports Layers
// 1-7 as one lump that is 71.4 pct (GB10) / 84.5 pct (RTX 4090) of wall time, which
// says nothing about WHICH layer dominates. That matters because the paper's
// Amdahl argument points at offloading those layers next, and the per-layer
// split says where to start.
//
// The timers are plain clock_gettime around straight-line CPU code, so the
// overhead is nanoseconds against milliseconds of work: unlike the CUDA event
// instrumentation, this one does not measurably perturb the run.
//
// ORIGINAL HEADER FOLLOWS
// INSTRUMENTED VARIANT -- derived mechanically from fsrcnn_gpu_main.cu.
//
// The numerics are byte-for-byte identical to fsrcnn_gpu_main.cu; do not use
// this binary for headline timings. It exists to answer two reviewer
// questions that the production binary cannot:
//
//   1. Table III lumps both kernels into a single gpu_l8_ms window that also
//      contains the 56 per-channel H2D copies issued inside it. This variant
//      brackets each deconv_kernel launch and the spatial_reduction_kernel
//      with their own CUDA events, so deconv / reduction / in-window transfer
//      can be reported separately.
//   2. Input geometry is hardcoded to 176x144x150 upstream. Here it is
//      env-overridable (FSRCNN_ROWS / FSRCNN_COLS / FSRCNN_FRAMES) so the
//      self-contained reconstruction-quality experiment can feed it an
//      88x72 downsampled sequence. Defaults are the original values, so with
//      no env vars set this reproduces upstream behaviour exactly.
//
// The events are recorded without an intervening host sync and drained only
// after the final cudaEventSynchronize, so added serialization is nil.
//
// Compile with: nvcc -arch=native -O3 -o fsrcnn_gpu_instr fsrcnn_gpu_instrumented.cu -lm
// ============================================================================
...
// --- instrumentation-only: per-kernel breakdown of the gpu_l8 window ---
#define MAX_CH8 64
double g_deconv_ms = 0.0; // sum of the 56 deconv_kernel launches
double g_reduce_ms = 0.0; // the single spatial_reduction_kernel
long g_deconv_launches = 0;

// --- per-layer CPU timing (Layers 1-7) ---
double g_layer_ms[7] = {0, 0, 0, 0, 0, 0, 0};
static cudaEvent_t g_ev_dec_a[MAX_CH8], g_ev_dec_b[MAX_CH8];
static cudaEvent_t g_ev_red_a, g_ev_red_b;
static int g_events_ready = 0;
...
void FSRCNN_Layer8_GPU(double *img_hr, double *img_fltr_7, int rows, int cols,
  ...
  // Instrumentation events are created once and reused across all frames.
  if (!g_events_ready) {
    for (int j = 0; j < MAX_CH8; j++) {
      CHECK_CUDA(cudaEventCreate(&g_ev_dec_a[j]));
      CHECK_CUDA(cudaEventCreate(&g_ev_dec_b[j]));
    }
    CHECK_CUDA(cudaEventCreate(&g_ev_red_a));
    CHECK_CUDA(cudaEventCreate(&g_ev_red_b));
    g_events_ready = 1;
  }
  if (num_channels8 > MAX_CH8) {
    fprintf(stderr, "instrumentation: num_channels8=%d exceeds MAX_CH8=%d\n",
            num_channels8, MAX_CH8);
    exit(1);
  }
  ...
  for (int j = 0; j < num_channels8; j++) {
    ...
    CHECK_CUDA(cudaEventRecord(g_ev_dec_a[j]));
  ...
  // Drain the per-kernel events now that everything up to ev_stop has
  // completed. No extra sync is needed, so this does not perturb the run.
  for (int j = 0; j < num_channels8; j++) {
    float t = 0.0f;
    CHECK_CUDA(cudaEventElapsedTime(&t, g_ev_dec_a[j], g_ev_dec_b[j]));
    g_deconv_ms += t;
    g_deconv_launches++;
  }
  {
    float t = 0.0f;
    CHECK_CUDA(cudaEventElapsedTime(&t, g_ev_red_a, g_ev_red_b));
    g_reduce_ms += t;
  }
...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale) {
  ...
  double t_layer = t_cpu_start; // per-layer stopwatch, reset at each boundary
  ...
  double *img_fltr_1 =
      (double *)malloc(rows * cols * num_filters * sizeof(double));
  double *kernel = (double *)malloc(filtersize * sizeof(double));
  double *img_fltr_p1 = img_fltr_1;

#pragma omp parallel for
  for (int i = 0; i < num_filters; i++) {
    imfilter(img_lr, weights_layer1 + i * filtersize,
             img_fltr_p1 + i * cols * rows, rows, cols, padsize);
    PReLU(img_fltr_p1 + i * cols * rows, rows, cols, biases_layer1[i],
          prelu_coeff_layer1);
  }

  g_layer_ms[0] += get_time_ms() - t_layer;
  t_layer = get_time_ms();
  ...
  double *img_fltr_2 =
      (double *)calloc(rows * cols * num_filters2, sizeof(double));
...
#pragma omp parallel for
  for (int i = 0; i < num_filters2; i++) {
    ...
    for (int j = 0; j < num_channels2; j++) {
      imfilter(img_fltr_1 + j * rows * cols,
               weights_layer2 + (i * num_channels2 + j) * filtersize2,
               img_fltr_2_tmp, rows, cols, padsize2);
      imadd(img_fltr_p2 + i * cols * rows, img_fltr_2_tmp, cols, rows);
    }
    PReLU(img_fltr_p2 + i * rows * cols, rows, cols, biases_layer2[i],
          prelu_coeff_layer2);
  }
  ...
  g_layer_ms[1] += get_time_ms() - t_layer;
  t_layer = get_time_ms();
  ...
  double *img_fltr_3 =
      (double *)calloc(rows * cols * num_filters3, sizeof(double));
...
#pragma omp parallel for
  for (int i = 0; i < num_filters3; i++) {
    ...
    for (int j = 0; j < num_channels3; j++) {
      imfilter(img_fltr_p2 + j * rows * cols,
               weights_layer3 + (i * num_channels3 + j) * filtersize3,
               img_fltr_3_tmp, rows, cols, padsize3);
      imadd(img_fltr_p3 + i * rows * cols, img_fltr_3_tmp, cols, rows);
    }
    PReLU(img_fltr_p3 + i * rows * cols, rows, cols, biases_layer3[i],
          prelu_coeff_layer3);
  }
  ...
  g_layer_ms[2] += get_time_ms() - t_layer;
  t_layer = get_time_ms();
  ...
  double *img_fltr_4 =
      (double *)calloc(rows * cols * num_filters4, sizeof(double));
...
#pragma omp parallel for
  for (int i = 0; i < num_filters4; i++) {
    ...
    for (int j = 0; j < num_channels4; j++) {
      imfilter(img_fltr_p3 + j * rows * cols,
               weights_layer4 + (i * num_channels4 + j) * filtersize4,
               img_fltr_4_tmp, rows, cols, padsize4);
      imadd(img_fltr_p4 + i * rows * cols, img_fltr_4_tmp, cols, rows);
    }
    PReLU(img_fltr_p4 + i * rows * cols, rows, cols, biases_layer4[i],
          prelu_coeff_layer4);
  }
  ...
  g_layer_ms[3] += get_time_ms() - t_layer;
  t_layer = get_time_ms();
  ...
  double *img_fltr_5 =
      (double *)calloc(rows * cols * num_filters5, sizeof(double));
...
#pragma omp parallel for
  for (int i = 0; i < num_filters5; i++) {
    ...
    for (int j = 0; j < num_channels5; j++) {
      imfilter(img_fltr_p4 + j * rows * cols,
               weights_layer5 + (i * num_channels5 + j) * filtersize5,
               img_fltr_5_tmp, rows, cols, padsize5);
      imadd(img_fltr_p5 + i * rows * cols, img_fltr_5_tmp, cols, rows);
    }
    PReLU(img_fltr_p5 + i * rows * cols, rows, cols, biases_layer5[i],
          prelu_coeff_layer5);
  }
  ...
  g_layer_ms[4] += get_time_ms() - t_layer;
  t_layer = get_time_ms();
  ...
  double *img_fltr_6 =
      (double *)calloc(rows * cols * num_filters6, sizeof(double));
...
#pragma omp parallel for
  for (int i = 0; i < num_filters6; i++) {
    ...
    for (int j = 0; j < num_channels6; j++) {
      imfilter(img_fltr_p5 + j * rows * cols,
               weights_layer6 + (i * num_channels6 + j) * filtersize6,
               img_fltr_6_tmp, rows, cols, padsize6);
      imadd(img_fltr_p6 + i * rows * cols, img_fltr_6_tmp, cols, rows);
    }
    PReLU(img_fltr_p6 + i * rows * cols, rows, cols, biases_layer6[i],
          prelu_coeff_layer6);
  }
  ...
  g_layer_ms[5] += get_time_ms() - t_layer;
  t_layer = get_time_ms();
  ...
  double *img_fltr_7 =
      (double *)calloc(rows * cols * num_filters7, sizeof(double));
...
#pragma omp parallel for
  for (int i = 0; i < num_filters7; i++) {
    ...
    for (int j = 0; j < num_channels7; j++) {
      imfilter(img_fltr_p6 + j * rows * cols,
               weights_layer7 + (i * num_channels7 + j) * filtersize7,
               img_fltr_7_tmp, rows, cols, padsize7);
      imadd(img_fltr_p7 + i * rows * cols, img_fltr_7_tmp, cols, rows);
    }
    PReLU(img_fltr_p7 + i * rows * cols, rows, cols, biases_layer7[i],
          prelu_coeff_layer7);
  }
  ...
  g_layer_ms[6] += get_time_ms() - t_layer;
...
int main(int argc, char *argv[]) {
  ...
  // Env overrides, defaulting to the values above so an unset environment
  // reproduces fsrcnn_gpu_main.cu exactly. Rows/cols must stay even: the
  // chroma path below indexes at half resolution.
  {
    const char *e;
    if ((e = getenv("FSRCNN_COLS")) && atoi(e) > 0)
      inCols = atoi(e);
    if ((e = getenv("FSRCNN_ROWS")) && atoi(e) > 0)
      inRows = atoi(e);
    if ((e = getenv("FSRCNN_FRAMES")) && atoi(e) > 0)
      num = atoi(e);
    if (inCols % 2 || inRows % 2) {
      fprintf(stderr, "FSRCNN_COLS/FSRCNN_ROWS must be even (got %dx%d)\n",
              inCols, inRows);
      return 1;
    }
  }
  ...
  fprintf(stderr,
          "[PROFILING] cpu_l17_ms=%.2f h2d_ms=%.2f gpu_l8_ms=%.2f d2h_ms=%.2f "
          "total_ms=%.2f vram_kb=%zu block_x=%d block_y=%d threads_reduce=%d\n",
          g_cpu_l17_ms, g_h2d_ms, g_gpu_l8_ms, g_d2h_ms, total_wall,
          g_vram_bytes / 1024, g_block_x, g_block_y, g_threads_reduce);

  // gpu_l8_ms brackets the whole launch sequence, so the residual below is
  // the H2D traffic issued *inside* that window (the 56 per-channel copies)
  // plus launch gaps. It is reported rather than hidden so the three parts
  // visibly sum back to gpu_l8_ms.
  double in_window_other = g_gpu_l8_ms - g_deconv_ms - g_reduce_ms;
  fprintf(stderr,
          "[KERNELSPLIT] deconv_ms=%.2f reduce_ms=%.2f in_window_other_ms=%.2f "
          "gpu_l8_ms=%.2f deconv_launches=%ld frames=%d dims=%dx%d\n",
          g_deconv_ms, g_reduce_ms, in_window_other, g_gpu_l8_ms,
          g_deconv_launches, num, inCols, inRows);

  {
    double sum = 0.0;
    for (int i = 0; i < 7; i++)
      sum += g_layer_ms[i];
    fprintf(stderr, "[LAYERSPLIT]");
    for (int i = 0; i < 7; i++)
      fprintf(stderr, " l%d_ms=%.2f", i + 1, g_layer_ms[i]);
    // sum_l1_l7 should track cpu_l17_ms closely; a gap means work sits between
    // the layer blocks and is being attributed to no layer at all.
    fprintf(stderr, " sum_l1_l7=%.2f cpu_l17_ms=%.2f residual_ms=%.2f\n", sum,
            g_cpu_l17_ms, g_cpu_l17_ms - sum);
  }
