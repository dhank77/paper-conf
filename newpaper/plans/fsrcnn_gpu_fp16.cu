// FSRCNN with GPU-accelerated Layer 8, reduced-precision variant.
// Layers 1-7 run on CPU in double, unchanged from fsrcnn_gpu_main.cu. Layer 8's
// private buffer, deconvolution weights, and MAC inputs are stored in
// half/bf16 instead of double; the inner product still accumulates in fp32,
// and the bias add plus final store back to img_hr stay in double, so this
// is a mixed-precision test of Layer 8 only, not the whole pipeline.
//
// This trades away the paper's bit-exactness guarantee for Layer 8: expect
// diff_bytes > 0 against the V1/V2 double-precision reference. Compare with
// PSNR/SSIM (as in Section IV of the paper), not cmp -l.
//
// Compile FP16 (default):
//   nvcc -arch=sm_90 -O3 -o fsrcnn_gpu_fp16 fsrcnn_gpu_fp16.cu -lm
// Compile BF16:
//   nvcc -arch=sm_90 -O3 -DUSE_BF16 -o fsrcnn_gpu_bf16 fsrcnn_gpu_fp16.cu -lm
// (On the RTX 4090 substitute -arch=sm_89.)
//
// Usage is identical to fsrcnn_gpu_main.cu:
//   ./fsrcnn_gpu_fp16 <input.yuv> <output.yuv> [block_x] [block_y] [threads_reduce]
...
// Precision toggle for Layer 8's private buffer and MAC inputs. Build with
// -DUSE_BF16 to switch from fp16 to bf16 (wider exponent range, coarser
// mantissa; useful if fp16 overflows/underflows on some channel).
#if defined(USE_BF16)
#include <cuda_bf16.h>
typedef __nv_bfloat16 half_t;
#define FLOAT2HALF(x) __float2bfloat16((float)(x))
#define HALF2FLOAT(x) __bfloat162float(x)
#define PRECISION_NAME "bf16"
#else
#include <cuda_fp16.h>
typedef __half half_t;
#define FLOAT2HALF(x) __float2half((float)(x))
#define HALF2FLOAT(x) __half2float(x)
#define PRECISION_NAME "fp16"
#endif
...
// Reduced-precision deconvolution: input plane, kernel weights, and the
// private per-channel buffer are half_t (fp16 or bf16). The MAC still
// accumulates in fp32 to avoid compounding rounding error over the 81 taps,
// matching how mixed-precision inference libraries handle this in practice;
// only the storage format, not the accumulator, is reduced.
__global__ void deconv_kernel_fp16(const half_t *__restrict__ d_input_padded,
                                   half_t *__restrict__ d_output,
                                   const half_t *__restrict__ d_kernel,
                                   int rows_pad, int cols_pad, int rows_out,
                                   int cols_out, int stride, int border,
                                   int fsize) {
    ...
    for (int kc = 0; kc < fsize; kc++) {
      ...
      if (i_candidate >= 0 && j_candidate >= 0 && i_candidate % stride == 0 &&
        ...
        if (i < rows_pad && j < cols_pad) {
          ...
          sum += HALF2FLOAT(d_input_padded[in_idx]) * HALF2FLOAT(d_kernel[k_idx]);
        }
      }
    }
  }

  d_output[out_y * cols_out + out_x] = FLOAT2HALF(sum);
}

// Reduction still accumulates in fp32 across the 56 half_t channel planes;
// bias add and the final store to d_img_hr happen in double so the output
// buffer's type matches the rest of the (unchanged) double-precision pipeline.
__global__ void spatial_reduction_kernel_fp16(const half_t *__restrict__ d_all_tmp,
  ...
  if (p < hr_pixels) {
    ...
    for (int j = 0; j < num_channels; j++) {
      sum += HALF2FLOAT(d_all_tmp[j * hr_pixels + p]);
    }
    d_img_hr[p] = (double)sum + bias;
  }
}
...
void FSRCNN_Layer8_GPU(double *img_hr, double *img_fltr_7, int rows, int cols,
  ...
  // Private buffer and MAC inputs are half_t now, so this is roughly half the
  // footprint of the double-precision baseline (d_img_hr and the H2D input
  // staging stay as-is below; only d_all_tmp/d_kernel8/d_input_padded shrink).
  g_vram_bytes = (rows_pad * cols_pad + (size_t)num_channels8 * hr_pixels +
                  (size_t)num_channels8 * filtersize8) *
                     sizeof(half_t) +
                 hr_pixels * sizeof(double);

  double t_h2d_start = get_time_ms();

  half_t *d_input_padded = NULL;
  half_t *d_all_tmp = NULL;
  half_t *d_kernel8 = NULL;
  double *d_img_hr = NULL;

  CHECK_CUDA(cudaMalloc(&d_input_padded, rows_pad * cols_pad * sizeof(half_t)));
  CHECK_CUDA(
      cudaMalloc(&d_all_tmp, num_channels8 * hr_pixels * sizeof(half_t)));
  CHECK_CUDA(
      cudaMalloc(&d_kernel8, num_channels8 * filtersize8 * sizeof(half_t)));
  CHECK_CUDA(cudaMalloc(&d_img_hr, hr_pixels * sizeof(double)));

  // One-time host-side cast of the Layer 8 weights (double -> half_t) before
  // the H2D copy; this is negligible next to the per-channel input transfer.
  half_t *h_kernel8_half =
      (half_t *)malloc(num_channels8 * filtersize8 * sizeof(half_t));
  for (int k = 0; k < num_channels8 * filtersize8; k++)
    h_kernel8_half[k] = FLOAT2HALF(weights_layer8[k]);
  CHECK_CUDA(cudaMemcpy(d_kernel8, h_kernel8_half,
                        num_channels8 * filtersize8 * sizeof(half_t),
  ...
  for (int j = 0; j < num_channels8; j++) {
    ...
    half_t *h_input_padded_half =
        (half_t *)malloc(rows_pad * cols_pad * sizeof(half_t));
    for (int k = 0; k < rows_pad * cols_pad; k++)
      h_input_padded_half[k] = FLOAT2HALF(h_input_padded[k]);
    free(h_input_padded);

    double t_copy_start = get_time_ms();
    CHECK_CUDA(cudaMemcpy(d_input_padded, h_input_padded_half,
                          rows_pad * cols_pad * sizeof(half_t),
                          cudaMemcpyHostToDevice));
    g_h2d_ms += (get_time_ms() - t_copy_start);
    free(h_input_padded_half);

    deconv_kernel_fp16<<<grid_deconv, block_deconv>>>(
  ...
  spatial_reduction_kernel_fp16<<<blocks_reduce, threads_reduce>>>(
...
int main(int argc, char *argv[]) {
  ...
  fprintf(stderr,
          "[PROFILING] precision=%s cpu_l17_ms=%.2f h2d_ms=%.2f gpu_l8_ms=%.2f "
          "d2h_ms=%.2f total_ms=%.2f vram_kb=%zu block_x=%d block_y=%d "
          "threads_reduce=%d\n",
          PRECISION_NAME, g_cpu_l17_ms, g_h2d_ms, g_gpu_l8_ms, g_d2h_ms,
          total_wall, g_vram_bytes / 1024, g_block_x, g_block_y,
          g_threads_reduce);
