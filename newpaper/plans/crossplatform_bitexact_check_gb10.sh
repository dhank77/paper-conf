...
# GB10 / ASUS Ascent GX10 variant. Run the matching RTX 4090 script
# (crossplatform_bitexact_check_rtx4090.sh) on the other machine.
#
...
TAG="gb10"
EXPECTED_GPU_PATTERN="GB10|Blackwell"
...
if command -v nvidia-smi &>/dev/null; then
    ...
    if ! echo "$GPU_NAME" | grep -qE "$EXPECTED_GPU_PATTERN"; then
        echo "WARNING: expected a GB10/Blackwell GPU but detected '$GPU_NAME'."
