...
To contextualize SyncPilot's performance, we compare against two baselines: (1) the serial FSRCNN execution from \cite{annisa2025fsrcnn}, which recorded 86s on Big CPU and 169s on LITTLE CPU for the full sequence, and (2) our own serial baseline execution on Big core pinned (49198 ms) and parallel OpenMP baseline (44730 ms, 3.35 FPS) using 8 threads with Linux CFS scheduling.

We compared SyncPilot against the following configurations:
\begin{itemize}
    \item Baseline (Serial): Serial FSRCNN execution on a single Big CPU (86s) and single LITTLE CPU (169s), as reported in \cite{annisa2025fsrcnn}.
    \item Baseline (OpenMP Parallel): Parallel FSRCNN using 8 threads via OpenMP ($\texttt{OMP\_NUM\_THREADS}=8$), relying on the Linux CFS for thread scheduling without explicit affinity.
    \item OpenMP Big-Only: 4 threads pinned to Big CPUs via OpenMP affinity, matching the best configuration from \cite{annisa2025fsrcnn} (24s).
    \item OpenMP LITTLE-Only: 4 threads pinned to LITTLE CPUs, representing the worst parallel scenario from \cite{annisa2025fsrcnn} (75s).
    \item OpenMP Hybrid: 8 threads across all cores (big.LITTLE and LITTLE.big ordering), both achieving 26s in \cite{annisa2025fsrcnn}.
    \item SyncPilot-4W: SyncPilot with 4 worker threads on Big cores.
    \item SyncPilot-8W: SyncPilot with 8 worker threads across all cores.
\end{itemize}
