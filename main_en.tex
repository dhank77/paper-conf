...
We validate our approach on an Orange Pi 5 platform (RK3588S SoC) using a Fast Super-Resolution Convolutional Neural Network (FSRCNN) \cite{dong2016fsrcnn}. Results show that SyncPilot achieves a 3.39$\times$ speedup over the baseline parallel implementation, with the 4-worker Big-core configuration yielding the best throughput at 10.74 FPS.
...
\item We evaluate 7 SyncPilot configurations on Orange Pi 5 using FSRCNN, demonstrating that the 4-worker Big-core-only setup achieves the best performance (3.39$\times$ speedup), and that all configurations produce numerically identical outputs.
\end{itemize}
...
We implemented SyncPilot in C using POSIX threads and evaluated it on an Orange Pi 5 board equipped with a Rockchip RK3588S SoC. The SoC features a heterogeneous configuration: 4x Cortex-A76 (Big Cores) @ 2.4 GHz and 4x Cortex-A55 (LITTLE Cores) @ 1.8 GHz, with 8 nm LP process technology. The operating system was Ubuntu 22.04 with Linux Kernel 5.10. Compilation was performed using GCC 11.3.0 with \texttt{-O3} optimization flags.
...
To contextualize SyncPilot's performance, we compare against two baselines: (1) the \textbf{serial FSRCNN} execution from \cite{annisa2025fsrcnn}, which recorded 86s on Big CPU and 169s on LITTLE CPU for the full sequence, and (2) the \textbf{OpenMP parallel baseline} using 8 threads with Linux CFS scheduling, evaluated in our environment.
...
\begin{table}[htbp]
\caption{Performance Comparison Across All Configurations}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Configuration} & \textbf{Avg. Time (ms)} & \textbf{Throughput (FPS)} & \textbf{Speedup} \\
\hline
Serial (Big CPU) \cite{annisa2025fsrcnn} & 86000 & 1.74 & 1.00$\times$ \\
Serial (LITTLE CPU) \cite{annisa2025fsrcnn} & 169000 & 0.89 & 0.51$\times$ \\
OpenMP Big-Only (4T) \cite{annisa2025fsrcnn} & 24000 & 6.25 & 3.58$\times$ \\
OpenMP LITTLE-Only (4T) \cite{annisa2025fsrcnn} & 75000 & 2.00 & 1.15$\times$ \\
OpenMP Hybrid (8T) \cite{annisa2025fsrcnn} & 26000 & 5.77 & 3.31$\times$ \\
\hline
Baseline (OpenMP, 8T, CFS) & 47345 & 3.17 & 1.00$\times$ \\
SyncPilot-4W & 13968 & 10.74 & 3.39$\times$ \\
SyncPilot-8W & 16923 & 8.86 & 2.80$\times$ \\
\hline
\end{tabular}
...
Table \ref{tab:throughput} presents the throughput results. The reference paper \cite{annisa2025fsrcnn} established baselines using OpenMP with explicit CPU affinity: serial execution on a single Big CPU took 86s, while a single LITTLE CPU required 169s. Their best parallel configuration (4 Big CPUs) achieved 24s ($3.58\times$ speedup over serial Big), while 4 LITTLE CPUs reached 75s ($2.25\times$ over serial LITTLE). The OpenMP Hybrid configuration (big.LITTLE and LITTLE.big ordering) achieved 26s, demonstrating that thread ordering in the affinity mask has negligible impact.

Our baseline parallel implementation using 8 OpenMP threads without explicit affinity achieved 47345 ms (3.17 FPS), as the Linux CFS scheduler does not account for heterogeneous core capabilities, leading to suboptimal thread-to-core mapping and frequent cache thrashing. This is notably slower than the affinity-pinned OpenMP results from \cite{annisa2025fsrcnn}, confirming that explicit thread placement is critical on AMP architectures.

SyncPilot-4W achieves the best performance with 10.74 FPS (13968 ms), corresponding to a \textbf{3.39$\times$ speedup} over our CFS baseline and a \textbf{1.72$\times$ improvement} over the OpenMP Big-Only result (24s) from \cite{annisa2025fsrcnn}. By using exactly 4 worker threads --- matching the number of Big cores --- SyncPilot eliminates the contention between Big and LITTLE cores and ensures that the most compute-intensive pipeline stages are handled by the highest-performance cores.

Interestingly, SyncPilot-8W (8 workers across all cores) achieves 8.86 FPS (2.80$\times$ speedup), which is lower than the 4-worker variant. This degradation mirrors the finding in \cite{annisa2025fsrcnn} where LITTLE CPUs caused a $3\times$ slowdown compared to Big CPUs (75s vs. 24s), confirming that the deconvolution layer in FSRCNN is too compute-intensive for the Cortex-A55 cores.

\subsection{Calibration Overhead Analysis}
A key contribution of SyncPilot is the minimization of scheduling overhead through the IC-RCE mechanism. The calibration is performed only once during the first run (profiling run). As shown in the benchmark data, the profiling run for SyncPilot-4W completed in 12242 ms, which is actually faster than several steady-state runs (e.g., Run 6 at 15520 ms and Run 8 at 15962 ms). This indicates that the overhead of measuring per-stage execution times via \texttt{clock\_gettime} is negligible --- less than 1\% of total execution time.
...
\begin{table}[htbp]
\caption{Per-Layer Execution Time Distribution (SyncPilot-4W)}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Layer} & \textbf{Stage Type} & \textbf{Time (ms)} & \textbf{\% of Total} \\
\hline
Layer 1 & Feature Extraction ($5\times5$) & 412 & 2.9 \\
Layer 2 & Shrinking ($1\times1$) & 387 & 2.8 \\
Layers 3--6 & Non-linear Mapping ($3\times3$) & 2845 & 20.4 \\
Layer 7 & Expanding ($1\times1$) & 398 & 2.9 \\
Layer 8 & Deconvolution ($9\times9$) & 9926 & 71.0 \\
\hline
\textbf{Total} & & \textbf{13968} & \textbf{100.0} \\
\hline
\end{tabular}
\label{tab:layers}
\end{center}
...
Table \ref{tab:layers} shows the per-layer execution time distribution for SyncPilot-4W. Layer 8 (deconvolution) accounts for 71.0\% of the total execution time (9926 ms out of 13968 ms), confirming it as the dominant bottleneck. This aligns with the findings in \cite{annisa2025fsrcnn}, where Layer 8 showed the lowest speedup across all parallel scenarios.

In the reference paper, Layers 1--7 achieved approximately $3\times$ speedup with 4 Big CPUs, while Layer 8 dropped significantly due to: (1) the $9\times9$ kernel size requiring substantially more multiply-accumulate operations per output pixel, (2) higher memory bandwidth demands during the upscaling from 176$\times$144 to 352$\times$288, and (3) potential race conditions when multiple threads write to adjacent pixel regions without proper synchronization. SyncPilot addresses the race condition issue through its pull-based worker pool with shared stage queues and reorder buffer, ensuring that each stage's output is correctly sequenced before being consumed by the next stage.

When using LITTLE cores (SyncPilot-8W), Layer 8's execution time increases disproportionately, as the Cortex-A55 cores (1.8 GHz) lack the computational throughput to handle the deconvolution workload efficiently. This mirrors the reference paper's observation that 4 LITTLE CPUs required 75s --- $3.1\times$ slower than 4 Big CPUs (24s) --- with Layer 8 being the primary contributor to this disparity.
...
SyncPilot-4W (4 workers on Big cores) consistently outperformed all other configurations. The exclusive use of Big cores eliminates the straggler effect that occurs when LITTLE cores are assigned heavy stages. The 3.39$\times$ speedup over the CFS baseline demonstrates that proper core-task matching is more effective than simply maximizing thread count. This finding extends the reference paper's result \cite{annisa2025fsrcnn}, where 4 Big CPUs achieved 24s ($3\times$ over serial Big at 86s), by further reducing the time to 13.97s through the pull-based worker pool that eliminates thread-per-stage bottlenecks.

SyncPilot-8W, despite using more workers, achieved only 2.80$\times$ speedup. The inclusion of 4 LITTLE cores (Cortex-A55) introduces a bottleneck: when a LITTLE-bound worker picks up a compute-intensive stage such as the deconvolution layer, it becomes a straggler that delays the entire pipeline. This is evidenced by the higher average time (16923 ms vs. 13968 ms for 4W) and the larger variance between runs (16449--17314 ms vs. 12242--15962 ms). This directly mirrors the reference paper's finding that 4 LITTLE CPUs required 75s --- over $3\times$ slower than 4 Big CPUs (24s) --- confirming the performance asymmetry between core types.
